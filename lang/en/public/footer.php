<?php

	// PIED DE PAGE - FOOTER

	$FOOTER_DESCRIPTION = "Une superbe description que Skayn nous fera lorsqu'il aura arrêter de bavé sur le site.";

	$FOOTER_SERVER_TITLE = "YOUR SERVER";
	$FOOTER_SERVER_MEMBER = "Dashboard";
	$FOOTER_SERVER_SERVER = "Add your server(s)";
	$FOOTER_SERVER_BOT = "Add the bot";
	$FOOTER_SERVER_SUBSCRIPTION = "Subscriptions";

	$FOOTER_LINKS_TITLE = "USEFULL LINKS";
	$FOOTER_LINKS_CAREERS = "Careeres";
	$FOOTER_LINKS_GUIDELINES = "Guideline;";
	$FOOTER_LINKS_TOS = "Terms of Services";
	$FOOTER_LINKS_THANKS = "Thanks";

	$FOOTER_PARTNERS = "PARTNERS";

	$FOOTER_COPYRIGHT = "&copy; Copyright <strong><span>DISCORDTOP</span></strong> 2020. All Rights Reserved";
	$FOOTER_CREDIT = "Designed by ";

	$FOOTER_DISCORD_BUTTON = "JOIN US ON DISCORD";

?>